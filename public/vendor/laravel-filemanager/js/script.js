...
function useFile(file_url) {
    ...
    function useThumbnail( url, file_path ) {
        // var file_name = file_path.match(/[^\/]+$/)[0];
        //show remove button
        $('#js-remove-thumbnail', window.parent.document).addClass('active');

        //set the value of the desired input to image url
        $('#thumbnail-input', window.parent.document).val(file_path).trigger('change');

        //set or change the preview image src
        $('#thumbnail-holder', window.parent.document).attr('src', url).trigger('change');
        $('#thumbnail-stub', window.parent.document).addClass('hidden');
        $('#thumbnail-holder-wrapper', window.parent.document).addClass('active');

        //close modal
        $('#modal-filemanager', window.parent.document).removeClass('active');
        $('#overlay', window.parent.document).removeClass('active');

        //clear localstorage
        localStorage.removeItem( 'is_thumbnail_upload' );
    }

    function useBookfile( url, file_path ) {
        // var file_name = file_path.match(/[^\/]+$/)[0];
        //show remove button
        $('#js-remove-book_file', window.parent.document).addClass('active');

        //set the value of the desired input to image url
        $('#book_file-input', window.parent.document).val(file_path).trigger('change');

        //set or change the preview image src
        // $('#book_file-holder', window.parent.document).attr('src', url).trigger('change');
        $('#book_file-stub', window.parent.document).addClass('hidden');
        $('#book_file-holder-wrapper', window.parent.document).addClass('active');

        //close modal
        $('#modal-filemanager-file', window.parent.document).removeClass('active');
        $('#overlay', window.parent.document).removeClass('active');

        //clear localstorage
        localStorage.removeItem( 'is_book_file_upload' );
    }
    ...
    var is_thumbnail_upload = localStorage.getItem('is_thumbnail_upload') || localStorage.getItem('is_thumbnail_upload') === 'true';
    var is_book_file_upload = localStorage.getItem('is_book_file_upload') || localStorage.getItem('is_book_file_upload') === 'true';

    if (window.opener || window.tinyMCEPopup || field_name || getUrlParam('CKEditorCleanUpFuncNum') || is_ckeditor || is_thumbnail_upload || is_book_file_upload) {
        if (window.tinyMCEPopup) { // use TinyMCE > 3.0 integration method
            ...
            useFckeditor2(url);
        } else if ( is_thumbnail_upload === 'true' ) {
            useThumbnail( url, file_path );
        } else if ( is_book_file_upload === 'true' ) {
            useBookfile( url, file_path );
